...
import RS_EC82024 as RS_EC82024

def run_annexD_1d_ec82024(
    metadata_path: str,
    ...
    Design_Spectrum = RS_EC82024.get_RS_EC82024(T, S_alpha_RP, S_beta_RP, Soil_Type_spectrum) * 980.665
    ...
    # Use stable 2D slicing for the matching domain
    dom_idx = Domain[0]
    SaX_dom = TX[dom_idx, :]   # (nTdom, nRecords)
    SaY_dom = TY[dom_idx, :]
    Target_dom = Design_Spectrum[dom_idx]

    for i in range(len(Filename)):
        SEE_X[i] = np.mean((SaX_dom[:, i] - Target_dom) ** 2)
        SEE_Y[i] = np.mean((SaY_dom[:, i] - Target_dom) ** 2)
    ...
    Sa_ind = T_one[Domain]
    PGA_ind = T_one[0,:]
    Target = 0.95*Design_Spectrum[Domain]
